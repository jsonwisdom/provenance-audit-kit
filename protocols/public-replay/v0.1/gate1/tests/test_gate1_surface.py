...
class Gate1SurfaceTests(unittest.TestCase):
    def _fixture(self):
        ...
        (raw_root / digest).write_bytes(body)
        ...
        return temp, manifest, raw_root, profile

    def test_explicit_immutability_verifier_can_satisfy_v06(self):
        temp, manifest, raw_root, profile = self._fixture()
        try:
            result = verify_gate1(
    ...
    def test_failure_receipt_blocks_v07(self):
        temp, manifest, raw_root, profile = self._fixture()
        try:
            failure = raw_root.parent.parent / "receipts" / "failures" / "SSA_FETCH_FAILURE_v0.1.jsonl"
